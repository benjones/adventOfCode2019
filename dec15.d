...
Pos nearestUnknown(Pos p, const ref char[Pos] map){

    
    for(int i = 1; ; ++i){
        Pos[] nearest;
        foreach(r; p.r - i .. p.r + i + 1){
            foreach(c; p.c -1 .. p.c + i + 1){
                auto other = Pos(r, c);
                if(other !in map){
                    nearest ~= other;
                }
        ...
        if(!nearest.empty){
            return nearest.minElement!( (a){return abs(a.r- p.r) + abs(a.c - p.c);});
        }
    }
}


int shortestPath(Pos b, ref Pos e, char[Pos] map){
    ...
    while(!q.empty){
        ...
        foreach(i; North .. East+1){
            auto neighbor = forward(n, i);
            if(neighbor !in map){
                //quit and go to the closest unknown found in the search
                //writeln("bailing to go to: ", neighbor);
                e = neighbor;
                cameFrom[neighbor] = n;
                q = [];
                break;
            }
            if(neighbor in map && map[neighbor] == '#'){
                continue;
            }
            if(neighbor !in seen){
                seen[neighbor] = true;
    ...
    if(p.r == b.r){
        return p.c < b.c ? West : East;
    } else {
        return p.r < b.r ? North : South;
    }
    
}

int shortestPathLength(const ref char[Pos] map, Pos b, Pos e){
...
void printMap(const ref char[Pos] map, Pos robotPos){
    
    Pos minp, maxp;
    foreach(k; map.byKey){
        minp.r = min(k.r, minp.r);
        minp.c = min(k.c, minp.c);
        maxp.r = max(k.r, maxp.r);
        maxp.c = max(k.c, maxp.c);
    }
...
void main(){
    ...
    auto insts = input.split(",").map!(to!long).array;

    char[Pos] map;
    Pos robotPos, oxygenPos;
    int dir = North;
    ...
    auto next = nearestUnknown(robotPos, map);
    foreach(step; 0 .. 2700){
        ...
        auto stat = outp.front;
        outp.popFront;

        if(stat == 0){
            map[forward(robotPos, dir)] = '#';
        } else if(stat == 1){
            robotPos = forward(robotPos, dir);
            if(robotPos !in map){
                map[robotPos] = ' ';
            }
        } else if(stat == 2){
        ...
        if(robotPos == next){
            next = nearestUnknown(robotPos, map);
        }

        //writeln("next: ", next);
        dir = shortestPath(robotPos, next, map);
        if(dir < 0){
            break;
        }
        ...
        //        if(oxygenPos != Pos(0,0)){
        //            writeln("oxygen distance now: ", shortestPathLength(map, Pos(0,0), oxygenPos));
        //        }
    }
    writeln("found oxygen at ", oxygenPos);
    printMap(map, robotPos);

    writeln(shortestPathLength(map, Pos(0,0), oxygenPos));
    ...
    while(true){
        ...
        foreach(k, v; map){
            if(v == 'o'){
                foreach(dir; North .. East + 1){
                    auto pos = forward(k, dir);
                    if(map[pos] == ' ' || map[pos] == 'S'){
                        map[pos] = 'x';
                        changed = true;
                    }
                }
        ...
        if(!changed) break;
        foreach(line; 0..4){ writeln();}
        ++steps;
        writeln("after step: ", steps);
        printMap(map, robotPos);


    }
    writeln("took ", steps, " steps");
